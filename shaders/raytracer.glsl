...
struct ray_march_hr {
    float d; // distance to the hit objedt
    uint p_id; // primitive hit
    bool invert_normal;
};
...
vec3 get_final_color(vec3 world_pos, vec3 light_dir, primitive prim, bool invert_normal){
            ...
            return albedo * (ambient + diffuse) + vec3(spec);
}
...
vec3 csg_span(ray r) {
  ...
  vec3 light_dir = normalize(u_light_dir); // Light direction
  ...
  if (sp > 0) {
        ...
        if (best_idx != -1) {
            ...
            vec3 world_hp = r.origin + r.dir * t; // world coordinate hit point
            color = get_final_color(world_hp, light_dir, prim, hit.invert_normal);
        }
    }
  return color;
}

ray_march_hr sdf_merge(ray_march_hr right, ray_march_hr left, uint op){
  // UNION: Min(A, B)
  if(op == OP_TYPE_OPUNION){ 
    return (right.d < left.d) ? right : left;
  }
...
float sdf_box(vec3 p){
  vec3 b = vec3(0.5); // unit box
  vec3 q = abs(p) - b;
  return length(max(q,0.0)) + min(max(q.x,max(q.y,q.z)),0.0);
}

float sdf_sphere(vec3 p){ // sdf for unit sphere.
  return length(p) - 1.0;
}
...
ray_march_hr mapSDF(vec3 p){
  ray_march_hr stack[16];
  int sp = 0;
  
  int num_ops = instructions.length();

  for(int i = 0; i < num_ops; i++) {
    instruction instr = instructions[i];
    float d = 1e10;
    if(instr.type == ID_OP_TYPE_PRIMITIVE){
      primitive prim = primitives[instr.id];

      float scale = get_min_scale(prim.transform); // need this to avoid weird scaling traveling errors.

      mat4 inv_m = inverse(prim.transform);
      vec3 point = (inv_m * vec4(p,1.0)).xyz;
      //vec3 point = p;

      float local_d = 0.0;
      if (prim.type == PRIMITIVE_TYPE_SPHERE) {
        local_d = sdf_sphere(point);
      }
      else if (prim.type == PRIMITIVE_TYPE_CUBE) {  
        local_d = sdf_box(point);
      }
      else if(prim.type == PRIMITIVE_TYPE_CYLINDER){  
        local_d = sdf_cylinder(point);
      }
      
      // transform back d
      d = local_d * scale;
          
      ray_march_hr hr; // record the (maybe)hit!!!
      hr.d = d;
      hr.p_id = i; // index of current primitive
      hr.invert_normal = false; // Default: Normals point out             
      stack[sp++] = hr;
    }else if(instr.type == ID_OP_TYPE_OPERATION){
      operation op = operations[instr.id];
      ray_march_hr hr1 = stack[--sp];
      ray_march_hr hr2 = stack[--sp];

      ray_march_hr hr = sdf_merge(hr1, hr2, op.type);
      stack[sp++] = hr;
    }
  }
...
void main() {
  ...
  for(int s = 0; s<samples; s++){
    vec2 jitter = vec2(
        ...
        // switch methods.
      if(u_rendering_mode == 0){
        average_color += csg_span(r);
      }else{
        average_color += csg_raymarch(r);
      }
  }
  
  vec3 finalColor = average_color/float(samples);
